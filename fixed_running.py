...
def select_desk_zone(event, x, y, flags, param):
    global DESK_ZONE, SELECTING_DESK

    if event == cv2.EVENT_LBUTTONDOWN and SELECTING_DESK:
        DESK_ZONE.append((x, y))
        print(f"Desk point added: {(x, y)}")

        if len(DESK_ZONE) == 4:
            DESK_ZONE = np.array(DESK_ZONE, dtype=np.int32)
            SELECTING_DESK = False
            print("Desk zone locked:", DESK_ZONE)
...
class InferenceWorker(threading.Thread):
    ...
    def run(self):
        ...
        while not stop_event.is_set():
            ...
            # Create window & mouse callback ONCE
            if not hasattr(self, "window_initialized"):
                cv2.namedWindow(cam_name)
                cv2.setMouseCallback(cam_name, select_desk_zone)
            ...
            for box, kpts in zip(pose_results.boxes, pose_results.keypoints):
                ...
                cx = int((X1 + X2) / 2)
                cy = int((Y1 + Y2) / 2)
                # Bottom-center point (feet approximation)
                foot_x = int((X1 + X2) / 2)
                foot_y = Y2 - 5
                ...
                # Check geofence ONLY after desk is finalized
                if SELECTING_DESK:
                    continue

                if not inside_geofence((foot_x, foot_y), DESK_ZONE):
                     continue
                ...
                if inside_geofence((foot_x, foot_y), CRITICAL_ZONE):
            ...
            # ==============================
            # STEP 4: DRAW TEMP POINTS
            # ==============================
            if isinstance(DESK_ZONE, list):
                for p in DESK_ZONE:
                    cv2.circle(frame, p, 6, (255, 255, 0), -1)

            # ==============================
            # STEP 5: DRAW FINAL DESK ZONE
            # ==============================
            if not SELECTING_DESK and isinstance(DESK_ZONE, np.ndarray):
                cv2.polylines(frame, [DESK_ZONE], True, (255, 0, 0), 2)
                cv2.putText(
                    frame,
                    "DESK ZONE",
                    (DESK_ZONE[0][0], DESK_ZONE[0][1] - 10),
                    cv2.FONT_HERSHEY_SIMPLEX,
                    0.8,
                    (255, 0, 0),
                    2
...
if __name__ == "__main__":
    CAMERA_URLS = {
        "Cam3": "rtsp://admin:MgMUiF@2022@192.168.10.214:554/Streaming/Channels/17"}
